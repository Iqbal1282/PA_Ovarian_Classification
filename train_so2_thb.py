...
# WandB Settings
project_title = "PA-SO2-Ovarian-Cancer-Classification-Washu2"
...
for fold in range(k_fold):
    ...
    model = MultiModalCancerClassifierWithAttention(num_modalities=2, out_dim=1, fusion_dim=64, backbone_name='resnet18', dropout_prob=0.0).to(device)
    ...
    for epoch in tqdm(range(max_epochs), leave= False):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if len(batch) == 2:
                ...
                x, x2, y = batch
                loss = model.compute_loss([x.to(device), x2.to(device)], y.to(device)) #, x2.to(device))
            loss.backward()
            optimizer.step()
            optimizer.step()
